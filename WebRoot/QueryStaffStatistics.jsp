...
<body>
		...
		<div class="container-fluid">
			<div id="pad-wrapper">
				...
				<div class="table-wrapper products-table section">
					...
					<form action="${APP_PATH}/resultStaffStatistics.html" name="fm" method="post">
						<div class="row-fluid filter-block">
							<div class="pull-right">
								...
								<label>分类：</label>
								<div class="ui-select">
									<select id="selectClassify" name="classify">
										<c:forEach items="${classifyList}" var="one">
											<option <c:if test="${one.key == classify}">selected="selected"</c:if> value="${one.key}">${one.value}</option>
										</c:forEach>
									</select>
								</div>
								<label>部门：</label>
								<div class="ui-select">
									<select id="selectBm" name="bm">
										<option>全部</option>
										<c:forEach items="${bmList}" var="one">
											<option <c:if test="${one.key == bm}">selected="selected"</c:if> value="${one.key}">${one.value}</option>
										</c:forEach>
									</select>
								</div>
					...
					<div class="row-fluid">
						<table class="table table-hover">
							<thead>
								<tr>
									<th class="span3"><span class="line"></span>人员</th>
							...
							<c:if test="${!empty staffVOList}">
								<tbody>
									<c:forEach items="${staffVOList}" var="one" varStatus="varCount">
										<!-- row -->
										<tr>
											<td class="description">${one.staffName}</td>
	...
	<script>
		function queryPage() {
			...
			fm.action = "${APP_PATH}/resultStaffStatistics.html";
			fm.submit();
		}
		function exportExcel() {
			<c:if test="${!empty staffVOList}">
				window.location = "${APP_PATH}/exportStaffStatistics.html";
			</c:if>
			<c:if test="${empty staffVOList}">
				alert('请先查询数据！');
			</c:if>
