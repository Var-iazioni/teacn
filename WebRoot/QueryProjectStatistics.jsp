...
<body>
		...
		<div class="container-fluid">
			<div id="pad-wrapper">
				...
				<div class="table-wrapper products-table section">
					...
					<form action="${APP_PATH}/resultProjectStatistics.html" name="fm" method="post">
					...
					<div class="row-fluid">
						<table class="table table-hover">
							<thead>
								<tr>
									<th class="span3" style="width: 250px;"><span class="line"></span>项目编号</th>
									<th class="span3" style="width: 550px;"><span class="line"></span>项目名称</th>
							...
							<c:if test="${!empty projectVOList}">
								<tbody>
									<c:forEach items="${projectVOList}" var="one" varStatus="varCount">
										<!-- row -->
										<tr>
	...
	<script>
		function queryPage() {
			...
			fm.action = "${APP_PATH}/resultProjectStatistics.html";
			fm.submit();
		}
		function exportExcel() {
			<c:if test="${!empty projectVOList}">
				window.location = "${APP_PATH}/exportProjectStatistics.html";
			</c:if>
			<c:if test="${empty projectVOList}">
				alert('请先查询数据！');
			</c:if>
