...
<body>
		...
		<div class="container-fluid">
			<div id="pad-wrapper">
				...
				<div class="table-wrapper products-table section">
					...
					<form action="${APP_PATH}/resultFunctionStatistics.html" name="fm" method="post">
					...
					<div class="row-fluid">
						<table class="table table-hover">
							...
							<c:if test="${!empty functionVOList}">
								<tbody>
									<c:forEach items="${functionVOList}" var="one" varStatus="varCount">
										<!-- row -->
										<tr>
											<td class="description">${one.functionName}</td>
	...
	<script>
		function queryPage() {
			...
			fm.action = "${APP_PATH}/resultFunctionStatistics.html";
			fm.submit();
		}
		function exportExcel() {
			<c:if test="${!empty functionVOList}">
				window.location = "${APP_PATH}/exportFunctionStatistics.html";
			</c:if>
			<c:if test="${empty functionVOList}">
				alert('请先查询数据！');
			</c:if>
